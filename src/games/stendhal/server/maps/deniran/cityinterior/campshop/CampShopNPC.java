...
public class CampShopNPC implements ZoneConfigurator {
 private static CampShopNPC instance;

 private static final Logger logger = Logger.getLogger(CampShopNPC.class);
 ...
 public static CampShopNPC getCampshopNPC() {
  if(instance != null) {
   return instance;
  }
  else {
   instance = new CampShopNPC();
  }
  return instance;
  }
 ...
 private void initNPC(final StendhalRPZone zone) {
  ...
  lender.addJob("I run the Deniran Camp Shop. Let me know if you want to #rent an outfit.");
 ...
 public List<DeniranOutfit> createOutfitList(){
  final List<DeniranOutfit> outfitList = new LinkedList<DeniranOutfit>() {{
     ...
     add(new DeniranOutfit("sleepingbag", OutfitType.SLEEPINGBAG,5000));
  }};
  return outfitList;
  ...
  final ShopSign catalog = new ShopSign(null, null, null, true) {
   ...
   public boolean onUsed(final RPEntity user) {
    ...
    if (user instanceof Player) {
     ...
     player.addEvent(new ShowOutfitListEvent("Deniran Camp Shop", lender.getName() + " rents out the following outfits", toSend.toString()));
